...
**Safe patterns:**
```js
element.textContent = userInput                     // plain text, never parsed as HTML
element.innerHTML = DOMPurify.sanitize(userInput)   // when HTML must be rendered
```
...
```python
# VULNERABLE
User(username=username, password=password).save()   # plaintext — no hash at all
