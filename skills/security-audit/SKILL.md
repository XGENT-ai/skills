...
- Checking changed code before a commit or deployment — especially auth, API, database,
...
- If the request is about changes (a diff, staged files, a pre-commit or pre-deploy check),
  scope to the changed files (`git diff`, `git diff --staged`). Still read the surrounding code
  they call into or are called from — a change is often only exploitable in context. Run
  Step 2 only if a dependency manifest or lock file changed
...
- **Go**: Check `go.sum`
- If available, run the ecosystem's audit tool (`npm audit`, `pip-audit`, or equivalent) — it
  reflects current advisories. These tools need network access to the package registry; if
  it's unavailable, fall back to the watchlist below and say the audit was offline
...
Scan ALL files in scope (including config, env, CI/CD, Dockerfiles, IaC) for:
...
- Passwords stored in plaintext or with a fast hash; weak password policy; no MFA on
  privileged accounts
...
**Security Misconfiguration & Logging**
- Permissive CORS, debug mode in production, missing security headers
  (framework specifics in `references/language-patterns.md`)
- Security events (failed logins, access-control denials, privilege changes) not logged,
  leaving attacks undetectable
...
For a snippet or diff scope, drop sections that don't apply (e.g. the dependency audit when
no manifest changed) rather than filling them with zeros.
...
External references for each finding's Reference line:
