...
> 提炼自门户仓库 `docs/SSO与App开发指引.md` §11/§15.3/§15.4（2026-07）（门户仓文件，App 自己的 repo 里没有；本文件已自包含，不必去找）。冲突时以门户仓库为准。
...
⚠️ **声明规则读写不分**：`exchangeTargets` 背书的是整个命名空间，`B.write` 与 `B.read` 同一条待遇——用户态持有目标 App 的**写** scope 是合法姿势，平台不按读写另行裁量（该不该给，由同意屏上的用户决定）。别拿另一个 App「只声明了读」当平台规则：那是它自己的业务选择（它的写面不代表用户），照抄会把一个本来合法的功能砍掉。
