...
`--wait` 同理会失效（它就是在轮询这个面），**别把它当成保险还照绿放行**。
...
| 版本号发上去了，过一阵又变回旧值 | 该 key 是**门户仓内 App**（清单定义在门户代码里），version 随门户部署被等值对齐回定义值——仓内 App 不走自助发版面。外部 App 自 APP-DELIVERY-3 起清单事实源在你仓，不存在这个症状；真遇到就是有人在门户控制台手改过，找平台管理员对账 |
